 ...
 *
 * Flow: connect to WiFi (STA, configured via Kconfig) -> open the camera in
 ...
 *
 * @note SSID/password come from CONFIG_WH_WIFI_* and are embedded in the
 *       resulting binary. Single client
...
************************macro define************************
***********************************************************/
#define WIFI_SSID WH_WIFI_SSID
#define WIFI_PSWD WH_WIFI_PASSWORD
...
/* Return one complete JPEG. The Mac bridge normally takes screenshots from
 * the live canvas, while this endpoint remains useful for diagnostics. */
static void __serve_snapshot(int fd)
{
    ...
    if (sg_jpeg_len > 0) {
        len = sg_jpeg_len;
        snapshot = (uint8_t *)Malloc(len);
        if (snapshot) {
            memcpy(snapshot, sg_jpeg_buf, len);
        }
    }
    tal_mutex_unlock(sg_frame_mutex);

    if (NULL == snapshot || 0 == len) {
        static const char *unavailable =
            "HTTP/1.1 503 Service Unavailable\r\n"
            "Content-Type: text/plain\r\nConnection: close\r\n\r\n"
            "camera frame unavailable";
        __send_all(fd, unavailable, (int)strlen(unavailable));
    ...
    int header_len = snprintf(header, sizeof(header),
                              ...
                              "Connection: close\r\n\r\n",
                              (unsigned)len);
    if (__send_all(fd, header, header_len) >= 0) {
        __send_all(fd, snapshot, (int)len);
    }
    Free(snapshot);
}
...
static void __handle_client(int fd)
{
    ...
    if (NULL != strstr(req, "GET /stream")) {
        __serve_stream(fd);
    } else if (NULL != strstr(req, "GET /snapshot")) {
        __serve_snapshot(fd);
    } else {
        __serve_index(fd);
    }
...
void user_main(void)
{
    ...
    if ('\0' == WIFI_SSID[0]) {
        PR_ERR("WiFi SSID is empty. Configure CONFIG_WH_WIFI_SSID before building.");
        return;
    }
